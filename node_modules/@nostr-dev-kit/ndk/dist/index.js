...
__export(index_exports, {
  ...
  DEFAULT_ENCRYPTION_SCHEME: () => DEFAULT_ENCRYPTION_SCHEME,
...
var NDKUser = class _NDKUser {
  ...
  get npub() {
    if (!this._npub) {
      ...
      this._npub = import_nostr_tools4.nip19.npubEncode(this.pubkey);
    }
    return this._npub;
  }
  get nprofile() {
    console.log("encoding with pubkey", this.pubkey);
    return import_nostr_tools4.nip19.nprofileEncode({
      pubkey: this.pubkey
    });
  }
  set npub(npub) {
  ...
  get pubkey() {
    if (!this._pubkey) {
      ...
      this._pubkey = import_nostr_tools4.nip19.decode(this.npub).data;
    }
    return this._pubkey;
  }
  /**
  ...
  async getArticle() {
    ...
    switch (articleTag[0]) {
      case "a":
        ...
        taggedBech32 = import_nostr_tools5.nip19.naddrEncode({ kind: parseInt(kind), pubkey, identifier });
        break;
      case "e":
        taggedBech32 = import_nostr_tools5.nip19.noteEncode(articleTag[1]);
        break;
      case "r":
...
var NDKPrivateKeySigner = class _NDKPrivateKeySigner {
  ...
  getConversationKey(recipient) {
    ...
    const recipientHexPubKey = recipient.pubkey;
    return import_nostr_tools6.nip44.getConversationKey(this._privateKey, recipientHexPubKey);
  }
  async nip44Encrypt(recipient, value) {
    const conversationKey = this.getConversationKey(recipient);
    return await import_nostr_tools6.nip44.encrypt(value, conversationKey);
  }
  async nip44Decrypt(sender, value) {
    const conversationKey = this.getConversationKey(sender);
    return await import_nostr_tools6.nip44.decrypt(value, conversationKey);
  }
  /**
